...
class RtmBot(object):
    def __init__(self, token, plugin_directory):
        self.last_ping = 0
        self.token = token
        self.bot_plugins = []
    ...
    def load_plugins(self):
        logging.info('loading plugins from ' + self.plugin_directory)
        for plugin in glob.glob(self.plugin_directory + '/*'):
            sys.path.insert(0, plugin)
            sys.path.insert(0, self.plugin_directory + '/')
        for plugin in glob.glob(self.plugin_directory + '/*.py') + \
                glob.glob(self.plugin_directory + '/*/*.py'):
            logging.info('plugins... ' + plugin)
